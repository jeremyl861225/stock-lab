...
⚠ **2026-09-21 註記：這份的定價算式已經過時，不要再照抄。**
   本檔用的是舊的兩點模型（base×(1±skew)、ev = p·up + (1−p)·dn）。
   2026-09-19 起一年期改由 models/price_1y.py 的對數常態統一導出，
   **沒有獨立的 skew**。照抄會在 P漲 完全沒動的情況下把 exp_ret 悄悄改掉
   （2026-09-21 實際發生：+3.08% → +1.70%，改完才發現）。
   新的範本見 judgments/revise_1y_20260921_2454.py。
